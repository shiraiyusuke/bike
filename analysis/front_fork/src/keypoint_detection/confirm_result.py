...
def drew_sample(model_dict, image_file, input_shape, out_dir_gray):
    image_name = image_file.split('/')[-1]
    X = dl.image_load2d(image_file, input_shape, 'tensor', out_dir_gray)
    y_predict_list = []
...
if __name__ == '__main__':
    ...
    input_shape = (400, 300)
    model_dir = root_dir + 'analysis/front_fork/model/each/' + str(input_shape[0]) + '_' + str(input_shape[1]) + '/'
    out_dir_gray = root_dir + 'analysis/front_fork/data/images/test_image/left/tmp/300_400/gray/'
    # 予測に利用するモデル指定
    model_dict = {}
    # point_list = ['left_top', 'left_middle', 'left_bottom']
    point_list = ['left_top', 'left_middle', 'left_bottom', 'right_top', 'right_middle', 'right_bottom']

    for point in point_list:
        if input_shape[0] == 200:
            model_dict[point] = model_dir + point + '/bike_conv_model_architecture_' + point + '_150_200.json ' + model_dir + point + '/left_frontfork_' + point + '_150_200_weights.h5'
        elif input_shape[0] == 400:
            model_dict[
                point] = model_dir + point + '/bike_conv_model_architecture_' + point + '.json ' + model_dir + point + '/bike_conv_model_' + point + '_weights.h5'
    ...
    # テスト画像の読み込み
    # image_file = '/usr/local/wk/git_local/bike/analysis/front_fork/data/images/test_image/ok_data/300_400/8501833B3014030600400_300_400.jpg'
    image_file = '/usr/local/wk/git_local/bike/analysis/front_fork/data/images/training_image/left_resize/035120161123_l_400_300.jpg'
    ...
    if not os.path.exists(image_file):
        print 'imageが存在しません。%s' % image_file
        exit(1)

    drew_sample(model_dict, image_file, input_shape, out_dir_gray)
